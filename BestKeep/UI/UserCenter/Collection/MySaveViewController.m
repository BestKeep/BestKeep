    ...
    [BKService GetGoodsCollectionlist:[NSString stringWithFormat:@"%ld",(long)pageSize] pageno:[NSString stringWithFormat:@"%ld",(long)pag_no] view:self.view callback:^(id obj, NSError *error) {
        ...
        if (error) {
            ...
            [obj enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                [self.collectionListSource addObject:obj];
            }];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if ([editBtn.currentTitle isEqualToString:@"编辑"]) {
        ...
        [paramaDict setObject:goods.goods_no forKey:@"goodsNo"];
        [paramaDict setObject:goods.goods_name forKey:@"title"];
        NSString * jsonStr = [Common dictionaryToJson:paramaDict];
        NSString *strUrl = [NSString stringWithFormat:@"OBJC.executeJs(\"init\",%@)",jsonStr];
        
        OneViewController  * oneVC = [[OneViewController alloc] init];
        oneVC.hasShop = @"has";
        oneVC.myTitle = goods.goods_name;
        oneVC.oneUrl = strUrl;
        oneVC.oneRequest = strSave_detail;
...
- (void)deleteBtnClick:(UIButton*)sender //删除按钮
{
    ...
    [BKService deleteCollectGoods:[self getSelectedOrdersId:selectedModelArr] view:nil callback:^(id obj) {
        [self refreshData];
    }];
}
