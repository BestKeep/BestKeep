...
-(void)requestOrderCount{
    ...
    [BKService GetOrderStatusAmount:nil callback:^(id obj) {
    ...
    if (buttonIndex == 1) {
        ...
        if (![money isEqualToString:@""]) {
            ...
            if ([self checkRechargeMoney:[money integerValue]]) {
                [PassportService rechargeMoneyWithMoney:money callback:^(id obj) {
...
-(BOOL)checkRechargeMoney:(NSInteger)money{
    ...
    if (money<100) {
//        [ShowMessage showMessage:@"充值金额需大于100"];
        [ShowMessage showMessage:@"充值金额需大于100" withCenter:self.view.center];
        return NO;
    }else if (money >99999999){
        [ShowMessage showMessage:@"充值金额需小于99999999" withCenter:self.view.center];
        return NO;
    }else if (money%100 !=0){
        [ShowMessage showMessage:@"充值金额必须是100的整数" withCenter:self.view.center];
        return NO;
    }
